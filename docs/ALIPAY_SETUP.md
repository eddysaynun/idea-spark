...
Idea Spark 使用支付宝开放平台“当面付”预下单，为每笔订单生成独立动态二维码。固定商家收钱码不能提供可验签的逐订单通知，不参与自动发额。
...
4. 支付网关调用 `alipay.trade.precreate`，验签支付宝响应后返回动态二维码内容。
...
5. 用最低价格套餐完成真实付款，确认 D1 订单只发额一次、重复通知返回成功但不重复加额。
